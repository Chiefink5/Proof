...
function attachSwipe(surface, groupId, choreId) {
  let startX = 0;
  let currentX = 0;
  let active = false;
  const threshold = 90;
  ...
  function reset(animated = true) {
    surface.classList.toggle("swiping", !animated);
    if (animated) surface.classList.remove("swiping");
  ...
  surface.addEventListener("touchend", () => {
    ...
    if (currentX > threshold) {
      ...
      addLog({ groupId, choreId, action: "plus", date: parts.date, time: parts.time, manual: false });
    } else if (currentX < -threshold) {
      const parts = todayParts();
      addLog({ groupId, choreId, action: "minus", date: parts.date, time: parts.time, manual: false });
    } else {
      reset(true);
    }
    currentX = 0;
  ...
  group.chores.forEach(chore => {
    ...
    card.innerHTML = `
      <div class="swipe-track">
        <div class="left">+ Log done</div>
        <div class="right">Log correction -</div>
      </div>
      <div class="chore-surface">
    ...
    const surface = card.querySelector(".chore-surface");
    attachSwipe(surface, group.id, chore.id);
  });
